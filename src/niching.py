...
from core.niche import NichePopulation
...
def main(doPlot: bool, graphicsOutdir: str, withBoundaries: bool, statService: IStatService, instanceNumber: int = 0):
  ...
  pop = NichePopulation(Config.GeneticAlgoTuning.mutationRate)
  sim = Simulation(pop) \
  ...
  try:
    if doPlot:
      anim = animation.FuncAnimation(fig, sim.run, interval = 1, cache_frame_data = False)
      plt.show()
    else:
      while True:
        sim.run()
